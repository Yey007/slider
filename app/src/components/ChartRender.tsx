...
function draw(
  ...
  function line(start: Point<"screen">, end: Point<"screen">) {
    ctx.beginPath();
  ...
  function circle(point: Point<"screen">, radius: number) {
    ctx.beginPath();
    ctx.arc(point.x, point.y, radius, 0, 2 * Math.PI);
    ctx.stroke();
  }

  function drawPoint(point: Point<"screen">) {
    circle(point, CONTROL_RADIUS);
  }

  function drawControlPoint(
    controlPoint: Point<"screen">,
    relatedEndpoint: Point<"screen">
  ) {
  ...
  function drawCurve(curve: BezierCurve<"screen">) {
    ctx.beginPath();
...
const ChartRender: React.FC<ChartRenderProps> = ({
  curves,
  canvasDimensions,
}) => {
  ...
  useEffect(() => {
    ...
    clear(context);
    fixDpr(context, canvasDimensions);
    draw(context, canvasDimensions, curves);
  }, [curves, canvasDimensions.width, canvasDimensions.height]);
